...
export const getLastFixturesForCompetition = async (
  id: CompetitionId,
  next
) => {
...
export const getNextFixturesForCompetition = async (
  id: CompetitionId,
  next
) => {
  const currentRound = await getCurrentRound(id);
...
const updateQueryToGetAllGroups = async (query, id, round, roundNumber) => {
  ...
  if (round === currentRound) {
    newQuery = {
      ...newQuery,
      'fixture.status.short': { $in: finishedMatchStatuses },
    };
  } else {
    newQuery = {
      ...newQuery,
      'fixture.status.short': { $nin: finishedMatchStatuses },
    };
  }
